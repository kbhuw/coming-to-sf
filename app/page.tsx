...
export default function Home(){
  ...
  const resetFilters=()=>{setQuery('');setCategory('All projects');setBusinessKind('all');setWindow('all');setPage(0);};
  ...
  actions.current={
    ...
    filter:async(c,w,k)=>{setQuery('');setBusinessKind(k||'all');setCategory(c);setWindow(w);setMobileList(false);await new Promise<void>(r=>requestAnimationFrame(()=>requestAnimationFrame(()=>r())));return {category:c,timing:w,businessKind:k||'all'};},
  ...
  const timingCounts=useMemo(()=>Object.fromEntries(WINDOWS.map(w=>[w.id,data?.projects.filter(p=>(category==='All projects'||p.category===category)&&(category!=='Food & shops'||businessKind==='all'||projectKind(p)===businessKind)&&(!query.trim()||[p.name,p.address,p.neighborhood,p.description,...p.sources.map(s=>s.reference)].join(' ').toLowerCase().includes(query.trim().toLowerCase()))&&windowFor(p,now)===w.id).length||0])),[data,now,category,businessKind,query]);
  ...
  return <main className="atlas">
    ...
    <div className="map-toolbar">
      ...
      {category==='Food & shops'&&<label className="timing-filter">Kind <select aria-label="Filter by business kind" value={businessKind} onChange={e=>{setBusinessKind(e.target.value);setWindow('all');}}><option value="all">All kinds</option>{Object.entries(BUSINESS_KINDS).map(([k,label])=>{const count=data?.projects.filter(p=>p.category==='Food & shops'&&projectKind(p)===k&&(!query.trim()||[p.name,p.address,p.neighborhood,p.description,...p.sources.map(s=>s.reference)].join(' ').toLowerCase().includes(query.trim().toLowerCase()))).length||0;return <option key={k} value={k} disabled={!count}>{label} ({count})</option>;})}</select></label>}
      ...
      {(category!=='All projects'||window!=='all'||query||businessKind!=='all')&&<button className="clear-filters" onClick={resetFilters}>Clear filters</button>}
