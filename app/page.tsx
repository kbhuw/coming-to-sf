...
export default function Home(){
  ...
  useEffect(()=>{let active=true;fetch('/projects.json').then(r=>{if(!r.ok)throw Error();return r.json() as Promise<Data>;}).then(d=>{if(active){d.projects=d.projects.map(p=>({...p,...(details as Record<string,object>)[p.id]}));setData(d);dataRef.current=d;}}).catch(()=>active&&setLoadError('Project data could not load. Please reload to try again.'));return()=>{active=false;};},[]);
  useEffect(()=>{
    let disposed=false;
    import('maplibre-gl').then(gl=>{
      ...
      map.on('load',async()=>{
        ...
        map.on('mousemove','project-points',e=>{const p=dataRef.current?.projects.find(p=>p.id===e.features?.[0]?.properties?.id);if(p)popup.setLngLat(e.lngLat).setText(p.name+' · '+kindLabel(p)+' · '+(p.date?fmt(p.date):'Date unknown')).addTo(map);});
  ...
  const filtered=useMemo(()=>{
    if(!data)return [];
    const q=query.trim().toLowerCase();return data.projects.filter(p=>(category==='All projects'||p.category===category)&&(category!=='Food & shops'||businessKind==='all'||projectKind(p)===businessKind)&&(window==='all'||windowFor(p,now)===window)&&(!q||[p.name,p.address,p.neighborhood,p.description,...p.sources.map(s=>s.reference)].join(' ').toLowerCase().includes(q))).sort((a,b)=>{
  ...
  },[data,query,category,window,now,businessKind]);
  ...
  actions.current={
    ...
    filter:async(c,w,k)=>{setBusinessKind(k||'all');setCategory(c);setWindow(w);setMobileList(false);await new Promise<void>(r=>requestAnimationFrame(()=>requestAnimationFrame(()=>r())));return {category:c,timing:w,businessKind:k||'all'};},
    read:async()=>({selected:selected?.id||null,visibleProjects:filtered.filter(p=>p.locations.some(c=>mapRef.current?.getBounds().contains(c))).slice(0,40).map(p=>({id:p.id,name:p.name,category:p.category,kind:kindLabel(p),date:p.date})),query,category,businessKind,timing:window,location:placeStatus,zoom:mapRef.current?.getZoom(),total:filtered.length,projects:filtered.slice(0,20).map(p=>({id:p.id,name:p.name,category:p.category,kind:kindLabel(p),date:p.date,summary:p.summary,recordLabel:p.recordLabel,arrivalLabel:p.arrivalLabel,evidenceUrl:p.evidenceUrl,sources:p.sources}))}),
    open:async(id)=>{const p=dataRef.current?.projects.find(p=>p.id===id);if(!p)throw Error('Project not found');choose(p);await new Promise<void>(resolve=>requestAnimationFrame(()=>requestAnimationFrame(()=>resolve())));return {id:p.id,name:p.name,date:p.date,summary:p.summary,recordLabel:p.recordLabel,arrivalLabel:p.arrivalLabel,evidenceUrl:p.evidenceUrl,sources:p.sources};}
  };
  useEffect(()=>registerMapTools(()=>actions.current),[]);
  const counts=useMemo(()=>Object.fromEntries(WINDOWS.map(w=>[w.id,data?.projects.filter(p=>windowFor(p,now)===w.id).length||0])),[data,now]);
  ...
  return <main className="atlas">
    ...
    <div className="map-toolbar">
      ...
      <div className="type-filters" aria-label="Filter by project type">{CATEGORIES.map(c=><button key={c} aria-pressed={category===c} onClick={()=>setCategory(c)}>{c!=='All projects'&&<CategoryIcon category={c}/>}{{'All projects':'All','Housing':'Homes','Food & shops':'Shops','Streets & transit':'Transit','Other places':'Other'}[c]}</button>)}</div>
      {category==='Food & shops'&&<label className="timing-filter">Kind <select aria-label="Filter by business kind" value={businessKind} onChange={e=>setBusinessKind(e.target.value)}><option value="all">All kinds</option>{Object.entries(BUSINESS_KINDS).map(([k,label])=>{const count=data?.projects.filter(p=>p.category==='Food & shops'&&projectKind(p)===k).length||0;return count>0?<option key={k} value={k}>{label} ({count})</option>:null;})}</select></label>}
      <label className="timing-filter">When <select aria-label="Filter by arrival time" value={window} onChange={e=>setWindow(e.target.value)}><option value="all">Any time</option>{WINDOWS.map(w=><option key={w.id} value={w.id}>{w.label}</option>)}</select></label>
      {category==='Food & shops'&&data&&!data.projects.some(p=>p.category==='Food & shops'&&p.date)&&<p className="undated-note">White icons = no exact arrival date. Listings include proposals and existing-business changes; read the status on each card.</p>}
    </div>
    <div className="workspace">
      <aside className={'project-browser '+(mobileList?'mobile-open':'')}>
        ...
        <div className="category-row" aria-label="Project categories">{CATEGORIES.map(c=><button key={c} aria-pressed={category===c} onClick={()=>setCategory(c)}>{c}</button>)}</div></div>
      ...
      <section className="map-section" aria-label="Map of upcoming San Francisco projects"><div ref={container} className="map"/>
        ...
        <div className="timing-key" aria-label="Timing colors"><strong>Color = arrival</strong>{WINDOWS.map(w=><button key={w.id} aria-pressed={window===w.id} onClick={()=>setWindow(window===w.id?'all':w.id)}><i style={{background:w.id==='unknown'?'white':w.color,border:'1px solid '+w.color}}/>{w.label}</button>)}</div>
