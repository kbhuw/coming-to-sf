export type MapActions = {
  ...
  filter:(category:string,timing:string,kind?:string)=>Promise<unknown>;
...
export function registerMapTools(getActions:()=>MapActions){
  ...
  const entries:Tool[]=[{
    ...
    name:'filter_projects',title:'Filter the map',description:'Set category and arrival timing in the visible map and list. All projects/all clears these filters.',inputSchema:{type:'object',properties:{category:{type:'string',enum:['All projects','Housing','Food & shops','Streets & transit','Other places']},timing:{type:'string',enum:['all','soon','year','later','unknown','past']},kind:{type:'string',enum:['all','cafe','restaurant','bar','retail','cannabis','health','childcare','fitness','hotel','office','storage','shops']}},required:['category','timing'],additionalProperties:false},annotations:{readOnlyHint:false,untrustedContentHint:false},
    execute:async(input)=>{const v=input as {category:string;timing:string;kind?:string};if(!v||!['All projects','Housing','Food & shops','Streets & transit','Other places'].includes(v.category)||!['all','soon','year','later','unknown','past'].includes(v.timing))throw Error('Invalid category or timing');if(v.kind!==undefined&&!['all','cafe','restaurant','bar','retail','cannabis','health','childcare','fitness','hotel','office','storage','shops'].includes(v.kind))throw Error('Invalid business kind');return actions.filter(v.category,v.timing,v.kind);}
  },{
